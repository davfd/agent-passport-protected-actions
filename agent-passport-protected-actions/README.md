...
1. [`src/demo.ts`](src/demo.ts) - the $425 allowed / $650 refused / delegation demo.
2. [`src/protected-action.ts`](src/protected-action.ts) - the main allow/refuse gate.
3. [`src/passport.ts`](src/passport.ts) - the agent passport object.
4. [`src/receipt.ts`](src/receipt.ts) - receipt hashing and writing.
